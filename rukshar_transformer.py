...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        # Split embeddings to self.heads diff. pieces
        ...
        # Einsum is for matrix mult.
        energy = torch.einsum("nqhd,nkhd->nhqk", [queries, keys])
        # queries shape:(N, query_len,heads,heads_dim)
        # keys shape: (N,key_len,heads,heads_dim)
        #energy: (N,heads, query_len, key_len)

        # Mask padded indices : the weights are 0
        if mask is not None:
            energy = energy.masked_fill(mask == 0, float("-1e20"))

        # Normalize energy vals to make the sum =1.
        # divide by scaling factor for scalability
        ...
        # reshape n' flatten the last 2 dim.

        out = self.fc_out(out)  # final shape:(N,query_len,embed_size)
        return out
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query, mask):
        ...
        # add skip conn., run thru normalization and finally dropout
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            src_vocab_size, embed_size, num_layers, heads, device, forward_expansion,
            dropout, max_length
        )
